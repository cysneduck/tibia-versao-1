-- Update claim_respawn function to remove user from queue after claiming
CREATE OR REPLACE FUNCTION claim_respawn(
  p_respawn_id UUID,
  p_character_id UUID
)
RETURNS JSON AS $$
...
BEGIN
  ...
  SELECT 
    CASE 
      WHEN ur.role = 'guild' THEN INTERVAL '1 hour 30 minutes'
